...
//        String entityname = getEntity(tablename);
            ...
            List bodyJson=queryResult(tablename,10);
            List bodyResult = new ArrayList(10);
            //获得表头
            List headResult=new ArrayList();
            try{
...
//            String sql ="select"+"\""+columnName+"\""+","+"count(*)"+"from "+"\""+tableName+"\""+"GROUP BY"+"\""+columnName+"\""+"ORDER BY "+"count(*)"+"DESC"+"limit 10";
            String sql ="select"+"\""+columnName+"\""+",count(*)"+"from "+"\""+tableName+"\""+"GROUP BY "+"\""+columnName+"\""+"ORDER BY count(*) DESC limit 5";
